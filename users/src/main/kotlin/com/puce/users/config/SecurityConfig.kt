...
import com.puce.users.security.CognitoClientIdValidator
import com.puce.users.security.CognitoTokenUseValidator
...
class SecurityConfig {
    ...
    fun securityFilterChain(
        ...
        .authorizeHttpRequests {
            it.requestMatchers("/actuator/health", "/error").permitAll()
                ...
                .requestMatchers(HttpMethod.PUT, "/users/me").hasAnyRole("STUDENT", "ADMIN")
    ...
    fun jwtDecoder(properties: CognitoProperties): JwtDecoder {
        ...
        decoder.setJwtValidator(
            DelegatingOAuth2TokenValidator(
                ...
                CognitoTokenUseValidator(),
                CognitoClientIdValidator(properties.clientId),
            ),
        )
        return decoder
    }
    ...
    private fun writeError(
        ...
        try {
            ...
            objectMapper.writeValue(response.outputStream, mapOf("timestamp" to Instant.now(), "status" to status, "error" to error, "message" to message, "path" to path))
        } finally {
